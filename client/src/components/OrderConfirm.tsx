import type { OrderConfirmProps, ConfirmItem } from "@/types";
...
export default function OrderConfirm({
  ...
  const fullItems: ConfirmItem[] = items.map((cartItem) => {
    const productMatch = products?.find((p) => p.name === cartItem.name);

    return {
      ...cartItem,
      image: productMatch?.image.thumbnail ?? "/images/default-thumbnail.jpg",
    };
  });
